...
- **REG-M03 Download Center**: architecture 固定为 `Browser -> Aureole -> Solution -> fixed GitHub Releases adapter`。Registry `download-center` snapshot 只保存严格验证的 public config；scheduled 在现有单一 `waitUntil` 内完成 Registry refresh 后，从 code-owned `https://api.github.com/repos/{owner}/{repo}/releases/latest` 解析 bounded metadata、执行 case-insensitive literal matcher、验证 selected `github.com/.../releases/download/...` URL，并将 normalized public metadata写入同一个 `REGISTRY_KV` 的 `registry:download-center:resolved:v1`。`GET /api/v1/downloads` 只读取该派生 LKG，不调用 GitHub/Admin/V2Board/mirror。GitHub failure 按 item 隔离，prior LKG 仅在 item `maxStaleHours` 内可用；removed/disabled/config-changed/expired item fail closed。Registry不能选择 outbound origin/API URL、regex、executable template 或 credential；mirror 只做 bounded HTTPS presentation URL rendering，Solution从不 fetch mirror。该状态可重建，不包含业务/用户权威、raw provider payload 或 secret，也不改变 V2Board authority。
