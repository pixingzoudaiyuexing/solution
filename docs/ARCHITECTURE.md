...
- **REG-M03 Download Center**: architecture 固定为 `Browser -> Aureole -> Solution -> fixed GitHub Releases adapter`。Registry `download-center` snapshot 保存严格验证的 items、module-level `github-url-prefix` providers 与 exactly-two default provider IDs；scheduled 在现有单一 `waitUntil` 内完成 Registry refresh 后，从 code-owned `https://api.github.com/repos/{owner}/{repo}/releases/latest` 解析 bounded metadata、执行 case-insensitive literal matcher、验证 selected `github.com/.../releases/download/...` URL，并生成两个 literal provider-prefix URLs。GitHub adapter 保存可注入 fetch 时使用 receiver-safe arrow wrapper，使底层 native/injected fetch 始终以 bare-call receiver执行；禁止把 native fetch作为 adapter method调用。可选 `GITHUB_API_TOKEN` 仅来自 deployment secret并只注入固定 GitHub request，Registry/Browser不能选择或读取它，token 不进入持久化、Public DTO、log 或 error。单次 run 对 normalized repository 使用 bounded in-memory Promise cache，canonical seven-item config 只解析三个 repositories。`GET /api/v1/downloads` 只读取 `registry:download-center:resolved:v1` schema-v2 派生 LKG，不调用 GitHub/Admin/V2Board/provider；旧 schema-v1 payload fail closed。Public DTO 只有 `downloads[2]`，没有 standalone original GitHub URL、provider base config或旧 `mirrors`。Provider config/order 参与 effective fingerprint；repository/provider failure按边界隔离，prior LKG 仅在相同 fingerprint 与 item `maxStaleHours` 内可用。Internal `registry:download-center:diagnostic:v1` 只为本次 scheduled run 实际请求的 unique normalized repositories 保存 bounded success/error class、attempt timestamp 与 elapsed milliseconds；fetch rejection、manual redirect和其他 HTTP error使用分离的 strict code，只有真实 HTTP Response 才可保存 bounded `httpStatus`。它独立可损坏、best-effort 持久化，不是功能权威，不影响 resolved LKG 或 Public API。Windows/Mac/Android 为单 items，多个 `platform=linux` items 由 Aureole分组为 `Linux GUI`；iOS保留未来 separately approved non-download destination，当前不进入 Public Contract。所有 M03 operational state 都可重建，不包含业务/用户权威、raw provider payload、response body/header、provider URL、secret、stack、cause 或 error message，也不改变 V2Board authority。
