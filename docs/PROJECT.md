...
SOL-REG-M04-M06-01 implementation candidate runtime/code anchor is `96bfcb37ffa02589ee2566399a0f19816ac999b6`. It adds REG-M04 `subscription-delivery`, authenticated delivery-options/access-link APIs and Registry-independent root bearer streaming. It preserves existing previous-purchaser eligibility, proves Official normal token equality before generating new links, keeps all CF-02 and legacy access contracts unchanged, and implements REG-M06 `info=hide` as header-only suppression. `/api/v1` route count becomes 50; M05, Aureole migration, V2Board modification and Production provisioning/deployment are not implemented. Candidate awaits mandatory Gemini post-code review.
