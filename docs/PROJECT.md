...
A3 `SOL-REG-KERNEL-03` implementation candidate 的 runtime/code anchor 为 `fa5b7714c7ecd61b49a977729a7ec652fb8e61ab`，当前等待 mandatory Gemini post-code review，尚未 closed/re-frozen。A3 只注册 REG-M01 `runtime-settings`，以 code-owned 24-hour `STALE_TOLERANT` policy 从现有 `REGISTRY_KV` safe snapshot 读取，并通过 anonymous `GET /api/v1/config/runtime` 返回显式七字段 `string|null` DTO。该 route 不触发 Admin Knowledge、Control Plane、refresh、V2Board 或 external fetch；不可用状态返回 all-null 200 fallback。当前 Public route count 为 48；REG-M07 与其他 product modules 未实现，Production 未部署或 provision 新资源。
