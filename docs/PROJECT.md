...
M03 GitHub Diagnostics runtime anchor 为 `e73ed87dea8c9d0dbf98854d36a238bc645981b2`，已 fast-forward 合入 main 并部署到 TEST Worker version `c806fb1d-a5e0-4c29-ba40-09dc8f40fae3`；Production 未部署。现有 diagnostic v1 已证明三个 repository attempts 都落入 broad `UPSTREAM_ERROR`。M03 GitHub Error Classification 当前为 local candidate，pending Antigravity Gemini review，未 merge、未 deploy；它将 fetch rejection、manual redirect 与其他 HTTP error细分为 strict internal codes，并仅在真实 Response 存在时保存 bounded status。该候选不改变 10 秒 timeout、retry、provider、refresh、resolved schema v2、LKG fallback 或 `/api/v1/downloads`，也不修复根因。
