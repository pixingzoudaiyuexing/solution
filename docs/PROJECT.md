...
M03 GitHub Diagnostics runtime anchor 为 `e73ed87dea8c9d0dbf98854d36a238bc645981b2`；reviewed error-classification anchor `1d6e00c907dc7b61b50ce87be625fb28d34f44a9` 已 fast-forward 合入 main并部署到 TEST Worker version `c594e348-bff9-4f37-993a-65ece95dac63`，Production 未部署。真实 due Cron 将三个 repository attempts 精确分类为 `FETCH_REJECTED / elapsedMs=0 / no Response`。Root-cause investigation 通过 actual local workerd 证明 native fetch 作为 object method调用会 `Illegal invocation`，而 bare/arrow invocation成功；当前 final receiver-fix 为 local candidate，pending Antigravity Gemini review，未 merge、未部署。修复只用 arrow wrapper保持 native/injected fetch 的 bare receiver，不改变 10 秒 timeout、retry、provider、refresh、resolved schema v2、LKG fallback 或 `/api/v1/downloads`。
