...
M03 receiver repair anchor `1814e0fe1c97684a70db2624f466b2e629eaf6d2` 已 fast-forward 合入 main并部署到 TEST Worker version `cf3d3413-0b4e-4144-9c32-fd52e6d12a51`，Production 未部署。真实 due Cron 已证明 receiver repair 使三个请求到达 GitHub，但三个 repository attempts 均返回 HTTP 403 `RATE_LIMITED`。隔离探针进一步证明 TEST-equivalent Smart Placement 出口的未认证 GitHub core quota 为 `60/remaining 0`，而非 Smart Placement 出口仍可 HTTP 200；临时 Worker 已删除。当前下一修复为 local candidate：保留 receiver-safe wrapper，增加 optional deployment-owned `GITHUB_API_TOKEN`，严格限制为 1..512 visible ASCII并只用于固定 GitHub request；不改变 10 秒 timeout、retry、provider、refresh、resolved schema v2、LKG fallback 或 `/api/v1/downloads`，且 token 不进入 Registry/KV/Public DTO/log/error。该候选 pending Antigravity Gemini review，未 merge、未 push、未部署，TEST Secret 未 provision。
