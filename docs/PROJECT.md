...
M03 GitHub Diagnostics 当前为 local implementation candidate，pending Antigravity Gemini local review，未 merge、未 deploy。它只新增 internal `registry:download-center:diagnostic:v1`，对 scheduled run 的 unique repository Promise记录 bounded success/error classification 与 elapsed time；不改变 10 秒 timeout、retry、provider、refresh、resolved schema v2、LKG fallback 或 `/api/v1/downloads`。Diagnostic KV 是独立 best-effort observational state，不是新 authority；根因尚未修复。
