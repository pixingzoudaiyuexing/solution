...
REG-M03 Download Center 当前为 implementation candidate，尚待 mandatory Gemini post-code review，未 merge、未 deploy。它注册 strict public `download-center` Registry module，scheduled 通过固定 GitHub Releases adapter解析 due items，并把 normalized、可重建的派生 LKG 写入同一个 `REGISTRY_KV` key `registry:download-center:resolved:v1`。Anonymous `GET /api/v1/downloads` 只读派生 LKG并返回 neutral DTO，不触发 Admin Knowledge、GitHub、V2Board 或 mirror fetch。当前 source route count 为 52；V2Board source/schema/business authority、Aureole consumer 和 Production 均未改变。
